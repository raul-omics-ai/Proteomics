...
auto_proteomic_analysis <- function(ms2_file_path, 
  ...
  # ms2 level
  clean_espectra_df <- evidence %>%
    filter(Reverse != "+",  # Para eliminar las proteínas decoy
  ...
  contrast <- apply(combn(groups, 2), 2, function(par) {
    paste(par, collapse = "_vs_")
  })
  
  for(pair in contrast){
    print(paste0("Contrast: ", pair))
    data_diff_all_contrasts <- test_diff(data_norm, type = "manual", test = pair)
    de_proteins <- add_rejections(data_diff_all_contrasts, 
                                  alpha = alpha, lfc = lfc)
    ...
    # Modify FDR calculation
    print("Adjusting P-values with BH procedure")
    data_results_clean <- data_results %>%
      arrange(.data[[pval_cols]]) %>%
      mutate(!!padj_cols := p.adjust(.data[[pval_cols]], method = "BH")) %>%
      select(
        name,
        ID,
      ...
      dplyr::rename(log2FC = all_of(ratio_col))
    
    data_results_clean <- data_results_clean %>%
      mutate(significant = ifelse(.data[[padj_cols]] < 0.05 & abs(log2FC) > 1, TRUE, FALSE))
    
    # Correcting DE_object
    
    de_data <- rowData(de_proteins) %>%
      as.data.frame() %>%
      arrange(.data[[pval_cols]]) %>%
      mutate(!!padj_cols := data_results_clean[[padj_cols]],
             !!sig_contrast := data_results_clean[["significant"]],
             significant = data_results_clean[["significant"]])
    
    idx <- match(rowData(de_proteins)$name, de_data$name)
    de_data <- de_data[idx, ]
    rowData(de_proteins) = de_data
    
    rm(de_cols, centered_cols, ratio_col)
    
    addWorksheet(wb, paste0("DE_Result", "_", pair))
    writeDataTable(wb, sheet = paste0("DE_Result", "_", pair), x = data_results_clean)
    ...
    save_ggplot(pval_distribution, title = paste0("Pvalue_Distribution_", pair), 
                folder = individual_plots_dir,
                width = 3000, height = 2000)
    
    # PCA
    n <- min(500, nrow(de_proteins))
    de_pca <- plot_pca(de_proteins, x = 1, y = 2, n = n, point_size = 4,
                       indicate = "condition", label = T) + 
      color_scale_color
    
    save_ggplot(de_pca, title = paste0("DE_Proteins_PCA_", pair), 
                folder = individual_plots_dir, 
                width = 3000, height = 2000)
    
    # Hierarchical clustering
    
    num_signif <- sum(rowData(de_proteins)$significant, na.rm = TRUE)
    
    if(num_signif > 5){
      print(paste0("There are ", table(rowData(de_proteins)$significant)[2], " differentially abundance proteins"))
      de_proteins_heatmap <- plot_heatmap(de_proteins, 
                                          type = "centered", kmeans = T, k = 2,
                                          show_row_names = T,
                                          indicate = c("condition"), plot = T)
      
      png(filename = file.path(output_dir, paste0("05_DE_Proteins_Heatmap_", pair,".png")), 
      ...
      save_ggplot(volcano_plot, title = paste0("DE_Proteins_VolcanoPlot_", pair), 
                  folder = individual_plots_dir,
                  width = 3000, height = 2000)
      ...
      save_ggplot(de_figure, title = paste0("DE_Result_", pair), 
                  folder = output_dir, width = 4500, height = 3000)
    }
    else{
      print(paste0("There are ", table(rowData(de_proteins)$significant)[2], " differentially abundance proteins"))
      
      #print("A total of 0 proteins were found differentially abundanced")
      print("Creating Figure")
      de_figure <- (cor_heatmap / (de_pca + pval_distribution))
      de_figure <- de_figure + plot_annotation(title = "Differential Abundance Analysis Result",
                                               tag_levels = "A",
                                               theme = theme(title=element_text(size = 20, hjust = 0.5, face = "bold")))
  ...
  } # Key for loop for individual contrast
